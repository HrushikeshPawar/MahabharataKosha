...
class Feedback(Base):
    ...
    shloka_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("shlokas.id"), nullable=False
    )
    language: Mapped[str] = mapped_column(
        Enum("english", "marathi", name="feedback_language_enum", create_type=False),
        nullable=False,
    )
    rating: Mapped[int] = mapped_column(
        Integer, nullable=False
    )  # e.g., 1 for "unhelpful", 5 for "very helpful"
